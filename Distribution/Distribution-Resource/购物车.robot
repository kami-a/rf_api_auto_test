...
添加商品到购物车
    [Arguments]    ${token}    ${distributionId}    ${distributionShopId}    ${distributionItemId}    ${itemid}    ${sellerId}    ${shopid}    ${skuid}    ${uid}
    ${resp}    Post    fmall    /mobile/shopCart/add    {"distributionRequest":{"distributionId":${distributionId},"distributionItemId":"${distributionItemId}","distributionShopId":"${distributionShopId}"},"product":{"qty":"9789","advanceActivitySellerId":"","depositValue":"","itemId":"${itemid}","payType":0,"quantity":1,"regionId":"11","sellerId":"${sellerId}","shopId":"${shopid}","skuId":"${skuid}"},"quantityNum":"undefined","uid":"${uid}"}    ${token}
    [Return]    ${resp}
